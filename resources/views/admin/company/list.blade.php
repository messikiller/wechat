...
@section('breadcrumb')
    <Breadcrumb>
        ...
        <Breadcrumb-item>公司列表</Breadcrumb-item>
    </Breadcrumb>
@endsection
...
@section('main-content')
    <Row>
        <i-col span="24">
            <i-button type="primary" icon="plus" onClick="window.location.href='{{ route('admin.company.add') }}'">添加公司</i-button>
        </i-col>
    </Row>
    ...
    data: {
        ...
        data: [
            @foreach ($list as $company)
            {
                ...
                'title': '{{ $company->title }}',
                'created_at': '{{ date('Y-m-d', $company->created_at) }}',
                'edit_url': '{{ route('admin.company.edit', $company->id) }}'
            }
                @if (! $loop->last)
                ,
                @endif

...
@section('breadcrumb')
    <Breadcrumb>
        ...
        <Breadcrumb-item>地区列表</Breadcrumb-item>
    </Breadcrumb>
@endsection
...
@section('main-content')
    <Row>
        <i-col span="24">
            <i-button type="primary" icon="plus" onClick="window.location.href='{{ route('admin.region.add') }}'">添加地区</i-button>
        </i-col>
    </Row>
    ...
    data: {
        ...
        data: [
            @foreach ($list as $region)
            {
                ...
                'title': '{{ $region->title }}',
                'created_at': '{{ date('Y-m-d', $region->created_at) }}',
                'edit_url': '{{ route('admin.region.edit', $region->id) }}'
            }
                @if (! $loop->last)
                ,
                @endif
